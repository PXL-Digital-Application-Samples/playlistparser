...
export default fp(async function authRoutes(fastify) {
  fastify.get('/auth/login', async (req, reply) => {
    ...
    reply.setCookie('oauth_state', state, {
      path: '/', httpOnly: true, sameSite: 'lax', secure: fastify.config.isProd
    });
  ...
  fastify.get('/auth/callback', async (req, reply) => {
    ...
    if (!cookieState || state !== cookieState) return reply.code(400).send({ error: 'bad_state' });
    ...
    const user = await prisma.user.upsert({
      ...
      create: { spotifyId: me.id, email: me.email ?? null, displayName: me.display_name ?? null },
    ...
    await prisma.token.upsert({
      ...
      update: {
        accessToken: tok.access_token,
        refreshEnc: encrypt(tok.refresh_token ?? decrypt((await prisma.token.findUnique({ where: { userId: user.id }}))?.refreshEnc || '')),
        scope: tok.scope,
        expiresAt: new Date(Date.now() + tok.expires_in * 1000)
      }
    });
    ...
    reply.setCookie('sid', user.id, {
      path: '/', httpOnly: true, sameSite: 'lax', secure: fastify.config.isProd, signed: true
    });
